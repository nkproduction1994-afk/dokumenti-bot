...
async def sacuvaj_fajl(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message.document:
        ...
        await update.message.reply_text("❌ Pošalji fajl (dokument, sliku, audio ili video).", parse_mode="Markdown")
        return SACUVAJ_FAJL
    ensure_files_dir()
...
async def lista_fajlova(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    fajlovi = load_fajlovi()
    if not fajlovi:
        await update.message.reply_text("📭 Nema sačuvanih fajlova.\n\nKoristi /sacuvaj da dodaš fajl.")
        return
...
async def fajlovi_dokumenta(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    args = ctx.args
    if not args:
        await update.message.reply_text("Koristi: `/fajlovi_dokumenta Naziv dokumenta`", parse_mode="Markdown")
        return
    naziv_trazeni = " ".join(args).strip().lower()
    fajlovi = load_fajlovi()
    filtrirani = [f for f in fajlovi if f.get("dokument", "").lower() == naziv_trazeni]
    if not filtrirani:
        await update.message.reply_text(f"📭 Nema fajlova vezanih za <b>{h(' '.join(args))}</b>.\n\nProvjeri naziv dokumenta sa /lista", parse_mode="HTML")
        return
...
async def podsjetnik_on(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    ctx.job_queue.run_daily(podsjetnik_job, time=time(hour=8, minute=0, second=0), name="jutarnji")
    await update.message.reply_text("🔔 *Jutarnji podsjetnici uključeni!*\n\nSvaki dan u *08:00* dobijaš poruku ako ima isteklih ili dokumenata koji uskoro ističu.", parse_mode="Markdown")
...
async def podsjetnik_off(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    for job in current_jobs:
        job.schedule_removal()
    await update.message.reply_text("🔕 *Jutarnji podsjetnici isključeni.*\n\nMožeš ih ponovo uključiti sa /podsjetnik_on", parse_mode="Markdown")
...
async def dodaj_naziv(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"📅 *Datum isteka za:* _{ctx.user_data['naziv']}_\n\nUnesi: `31.12.2025`", parse_mode="Markdown")
    return DATUM
...
async def dodaj_datum(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    ctx.user_data["in_conv"] = False
    await update.message.reply_text(f"✅ <b>Sačuvano!</b>\n\n📄 {h(doc['naziv'])}\n📅 {h(date_str)}\nStatus: {label}", parse_mode="HTML")
    return ConversationHandler.END
...
async def uskoro(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    docs = load_docs()
    today = datetime.today().replace(hour=0, minute=0, second=0, microsecond=0)
    filtered = [(d, (datetime.strptime(d["datum"], "%d.%m.%Y") - today).days) for d in docs if 0 <= (datetime.strptime(d["datum"], "%d.%m.%Y") - today).days <= 30]
...
async def isteklo(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    docs = load_docs()
    today = datetime.today().replace(hour=0, minute=0, second=0, microsecond=0)
    filtered = [(d, abs((datetime.strptime(d["datum"], "%d.%m.%Y") - today).days)) for d in docs if (datetime.strptime(d["datum"], "%d.%m.%Y") - today).days < 0]
    filtered.sort(key=lambda x: x[1])
    if not filtered:
...
async def ai_stop(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    ctx.user_data["ai_history"] = []
    await update.message.reply_text("👋 AI mod zatvoren. Historija razgovora obrisana.\n\nKoristi /ai za novi razgovor.", parse_mode="Markdown")
        ...
        if stripped.startswith("{") and '"action": "dodaj_dokument"' in stripped:
            try:
                ...
                if naziv and date_str:
                    ...
                    save_docs(docs)
                    label, _, _ = status(date_str)
                    await thinking_msg.edit_text(f"✅ <b>Dokument dodan putem AI!</b>\n\n📄 {h(naziv)}\n📅 {h(date_str)}\nStatus: {label}", parse_mode="HTML")
                    ctx.user_data["ai_history"] = history + [{"role": "user", "content": user_text}, {"role": "assistant", "content": f"Dodao sam dokument: {naziv}, datum: {date_str}"}]
                    return
            except json.JSONDecodeError:
                pass
        await thinking_msg.edit_text(response_text, parse_mode="Markdown")
        new_history = history + [{"role": "user", "content": user_text}, {"role": "assistant", "content": response_text}]
...
# IZMIJENJENA funkcija brzi_unos (sa ispravkom)
async def brzi_unos(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # AI mod aktivan (samo ako nema čekajućih interakcija)
...
def main():
    ...
    if settings.get("podsjetnici", False) and settings.get("chat_id"):
        app.job_queue.run_daily(podsjetnik_job, time=time(hour=8, minute=0, second=0), name="jutarnji")
